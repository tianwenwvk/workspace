 ...
 */
public class NioClientManager extends AbstractSecureNio implements ClientConnectionManager {
    ...
    class PendingConnect {
        ...
        PendingConnect(SSLEngine engine, SocketChannel sc, StreamConnection connection, SocketAddress address) { this.engine = engine; this.sc = sc; this.connection = connection; this.address = address; }
    }
    ...
    private final Set<SSLConnectionHandler> connectedSSLHandlers = Collections.synchronizedSet(new HashSet<SSLConnectionHandler>());
    ...
    private void handleKey(SelectionKey key) throws IOException {
        ...
        if (key.isValid() && key.isConnectable()) { // ie a client connection which has finished the initial connect process
            ...
            SSLEngine engine = data.engine;
            SocketChannel sc = (SocketChannel) key.channel();
            MessageWriteTarget target = null;
	        try {
	        	if (usingSSL) {
	        		SSLConnectionHandler sslhandler = new SSLConnectionHandler(connection, key, connectedSSLHandlers, engine);              
	        		target = sslhandler;
	        		while (!sc.finishConnect()) {
	        			// log.
	        		}
	        		engine.beginHandshake();
	        		if(sslhandler.doHandshake(sc, engine))
	        		{
	        			log.info("Connected to {} using ssl", sc.socket().getRemoteSocketAddress());
	                    key.interestOps((key.interestOps() | SelectionKey.OP_READ) & ~SelectionKey.OP_CONNECT).attach(sslhandler);
	                    connection.connectionOpened();
	                    data.future.set(data.address);
	        		} else {
	                    log.warn("Failed to connect to {} using ssl", sc.socket().getRemoteSocketAddress());
	                    sslhandler.closeConnection(); // Failed to connect for some reason
        ...
        try {
            ...
            for (SelectionKey key : selector.keys()) {
                ...
                if (key.attachment() instanceof ConnectionHandler)
                    SSLConnectionHandler.handleKey(key); // Close connection if relevant
                else if (key.attachment() instanceof SSLConnectionHandler)
                    SSLConnectionHandler.handleKey(key); // Close connection if relevant
            }
            try {
                selector.close();
    ...
    public ListenableFuture<SocketAddress> openConnection(SocketAddress serverAddress, StreamConnection connection) {
        ...
        try {
            ...
            sc.connect(serverAddress);//TODO
    		SSLEngine engine = null;
    		if(usingSSL){
    			String peerHost = sc.socket().getInetAddress().getHostAddress();
                int peerPort = sc.socket().getPort();
    			engine = setupEngine(peerHost, peerPort);
    		}
            PendingConnect data = new PendingConnect(engine , sc, connection, serverAddress);
    ...
    public int getConnectedClientCount() {
        return connectedSSLHandlers.size();
    }

    @Override
    public void closeConnections(int n) {
        while (n-- > 0) {
            SSLConnectionHandler handler;
            synchronized (connectedSSLHandlers) {
                handler = connectedSSLHandlers.iterator().next();
            }
            if (handler != null)
