...
/** peer certificate location **/
static std::string peer_certificate_file;
static std::string peer_private_key_file;
static std::string peer_ca_file;
...
{
	if(peer_certificate_file.empty())
		peer_certificate_file = GetArg("-peercertificatefile", SERVER_CERT_PATH);
	LogPrintf("-------GetArd %s -----GetPeerCerts %s\n", GetArg("-peercertificatefile", SERVER_CERT_PATH),peer_certificate_file);

	if(peer_private_key_file.empty())
		peer_private_key_file = GetArg("-peerprivatekeyfile", SERVER_KEY_PATH);
	if(peer_ca_file.empty())
		peer_ca_file = GetArg("-peercafile", CAFILE);
}
...
static bool sslctx_s_init()
{
	...
	g_s_sslctx = SSL_CTX_new(SSLv23_server_method());
	if (!g_s_sslctx) {
		LogPrintf("Unable to create SSL context");
	...
	SSL_CTX_load_verify_locations(g_s_sslctx, peer_ca_file.c_str(), NULL);
	/* Set the key and cert */

	if (SSL_CTX_use_certificate_file(g_s_sslctx, peer_certificate_file.c_str(), SSL_FILETYPE_PEM)< 0) {
		ERR_print_errors_fp(stderr);
		return false;
	}
	LogPrintf("sslctx_s_init() peer_certificate_file: %s\n", peer_certificate_file);
	if (SSL_CTX_use_PrivateKey_file(g_s_sslctx, peer_private_key_file.c_str(), SSL_FILETYPE_PEM)< 0) {
		ERR_print_errors_fp(stderr);
		return false;
	}
	...
	SSL_CTX_set_client_CA_list(g_s_sslctx, SSL_load_client_CA_file(peer_ca_file.c_str()));
...
static bool sslctx_c_init()
{
    ...
    if(g_c_sslctx == NULL){
    	LogPrintf("Unable to create client SSL context");
    	ERR_print_errors_fp(stderr);
        return false;
    }
    ...
    SSL_CTX_load_verify_locations(g_c_sslctx,peer_ca_file.c_str(), NULL);

    //load user certificate,this cert will be send to server for server verify
    if(SSL_CTX_use_certificate_file(g_c_sslctx,peer_certificate_file.c_str(),SSL_FILETYPE_PEM) <= 0){
        LogPrintf("certificate file error! %s\n", peer_certificate_file);
        return false;
    }
    LogPrintf("sslctx_c_init() peer_certificate_file: %s\n", peer_certificate_file);

    if(SSL_CTX_use_PrivateKey_file(g_c_sslctx, peer_private_key_file.c_str(),SSL_FILETYPE_PEM) <= 0){
...
//    if(g_s_sslctx == NULL){
//    	sslctx_s_init();
//    	LogPrintf("Debug -----BindListenPort %s sslctx init!\n", addrBind.ToString());
//    }
...
void StartNode(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // Start threads
    //
    LogPrintf("---------------LoadCertPath called from StartNode!");
    LoadCertsPath();
