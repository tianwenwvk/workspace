 ...
 * process data.
 */
public class NioServer extends AbstractSecureNio {
    ...
    private void handleKey(Selector selector, SelectionKey key) throws IOException {
        if (key.isValid() && key.isAcceptable()) {
            ...
            try {
            	// wrap in container
                if (usingSSL) {
                    peerHost = newChannel.socket().getInetAddress().getHostAddress();
                    peerPort = newChannel.socket().getPort();
                    SSLEngine engine = setupEngine(peerHost, peerPort);
                   //socket = new SecureSocket(socketChannel, engine, singleThreaded,taskWorker, toWorker, this, this);
                    SSLConnectionHandler sslhandler = new SSLConnectionHandler(connectionFactory, newKey, engine);
	                newKey.attach(sslhandler);
	                sslhandler.connection.connectionOpened();
                } 
//                else {              
...
//                }
    ...
    public NioServer(final StreamConnectionFactory connectionFactory, InetSocketAddress bindAddress) throws IOException {
    	this(connectionFactory, bindAddress, true, false, true);  	
    }

    /**
     * Create a new Server instance with ssl configured.
     *
    ...
    public NioServer(final StreamConnectionFactory connectionFactory, InetSocketAddress bindAddress,
    		boolean usingSSL, boolean isClient, boolean needClientAuth) throws IOException {
    	super(bindAddress, usingSSL, false, needClientAuth);
    	
    	if (usingSSL) {
            String trustStoreLoc = null;
            char[] tsPassPhrase = null;
            if(needClientAuth){
                trustStoreLoc = "servertrust.jks";
                tsPassPhrase = "123456".toCharArray();
            }
            setupSSL(trustStoreLoc, "serverkey.jks", tsPassPhrase, "123456".toCharArray());
        }
