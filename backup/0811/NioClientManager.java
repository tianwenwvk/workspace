   ...
   // private final Set<ConnectionHandler> connectedHandlers = Collections.synchronizedSet(new HashSet<ConnectionHandler>());
    ...
    private void handleKey(SelectionKey key) throws IOException {
        ...
        if (key.isValid() && key.isConnectable()) { // ie a client connection which has finished the initial connect process
			...
			SSLConnectionHandler handler = new SSLConnectionHandler(connection, key, connectedSSLHandlers, engine);              

//          handler = new ConnectionHandler(connection, key, connectedHandlers);

	        try {
	                if (handler.finishConnect()){	
	                	handler.processHandshake();
	                	//if(handler.isHandShakeComplete()) {  //if (sc.finishConnect()) {	                   
		                	log.info("Connected to {}", sc.socket().getRemoteSocketAddress());
		                	//handler.initHandshake();
...
//		                	handler.processHandshake();
    ...
    public NioClientManager(boolean usingSSL, boolean isClient, boolean needClientAuth) {
    	super(usingSSL, true, needClientAuth);
    	try {
    		if (usingSSL) {
                String keyStoreLoc = null;
                char[] ksPassPhrase = null;
                if (needClientAuth) {
                    keyStoreLoc = "serverkey.jks";
                    ksPassPhrase = "123456".toCharArray();
                }
                setupSSL("servertrust.jks", keyStoreLoc,
                        "123456".toCharArray(), ksPassPhrase);
            }
